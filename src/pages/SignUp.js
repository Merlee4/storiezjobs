    ...
    const createUser = (e) => {
        ...
        if(comfirm !== password){
            ...
            axios.post('http://localhost:9000/signup', {
                ...
                sessionStorage.setItem('createAccount', JSON.stringify({email:email, password:password}))
                setTimeout(() => {
                    router.push('/')
                },1000)
            }).catch(err => {
            ...
            <div className="bg-white w-6/12 mx-auto p-8 my-20">
                ...
                <form className="flex flex-col" onSubmit={(e) => createUser(e)}>
                    <div className="flex flex-col mb-4">
                        ...
                        <input type="text" value={names} onChange={(e) => setNames(e.target.value)} className="border-b-2  focus:border-blue-400"/>
                    </div>
                    
                    <div className="flex flex-col">
                        <label className="text-sm">Email</label>
                        <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="border-b-2  focus:border-blue-400"/>
                    </div>
                    ...
                    <div className="flex flex-col mt-4">
                        ...
                        <input type="password" value={comfirm} onChange={(e) => {
                                setComfirm(e.target.value)
                                setErrorMsg("")}} 
                        className="border-b-2  focus:border-blue-400"/>
                    </div>
                    <button className="bg-blue-500 my-4 py-2 text-white" type="submit">SignUp</button>
                    <Link to="/" className=" text-blue-400">already has an account Login</Link>
                </form>
            </div>
