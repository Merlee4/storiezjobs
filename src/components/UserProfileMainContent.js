  ...
  const editProfile = () => {
    ...
    axios
      .post("http://localhost:9000/editprofile", {
        firstname:
          newName.split(" ")[0].length > 1
            ? newName.split(" ")[0]
            : client.firstname,
        lastname:
          newName.split(" ")[1].length > 1
            ? newName.split(" ")[1]
            : client.lastname,
        location: newLocation.length > 2 ? newLocation : client.location,
        bio: newBio.length > 2 ? newBio : client.bio,
        about: newAbout.length > 2 ? newAbout : client.about,
        email: newEmail.length > 2 ? newEmail : client.email,
        profile: newProfile.length > 2 ? newProfile : client.profile,
        phone: newPhone.length > 2 ? newPhone : client.phone,
        website: newWebsite.length > 2 ? newWebsite : client.website,
      })
      .then((res) => {
        console.log(res);
      })
  ...
  return (
    <div>
      ...
      <div className="rounded-lg border py-4 px-8 bg-white text-gray-700 mt-4">
        ...
        <div className="mx-auto">{/* <JobsList /> */}</div>
      ...
      <Modal isOpen={editModal} className="">
        ...
        <div className=" ">
          ...
          <div className="flex flex-col">
            <input
              className="text-2xl my-2 font-medium text-gray-800"
        ...
        <div className="rounded-lg border py-4 px-8 bg-white text-gray-700 mt-4">
          ...
          <div className="flex justify-between gap-4 border p-4 rounded-xl">
            <div className="rounded-lg border p-4 my-4 w-full">
              ...
              <input
                ...
                className="text-blue-700 font-medium border-b"
                value={user.website}
                placeholder="website"
              />
              <p className="text-sm my-2">Location </p>
              <input
                className="p-2 rounded bg-gray-200 border-b"
            ...
            <div className="rounded-lg border p-4 my-4 w-full">
              ...
              <p className="text-sm">Phone </p>
              <input
                className="text-blue-700 hover:underline font-medium border-b"
          ...
          <button
            onClick={() => editProfile()}
            className="w-full bg-blue-500 text-white font-bold mt-4 rounded-sm py-2 px-1"
          >
            Edit
          </button>
        </div>
